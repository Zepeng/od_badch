...
int main(int argc, char** argv){
  ...
  kzinit_();
  //skoptn_("31,30,29,27,26,25,23,16", int(strlen("31,30,29,27,26,25,23,16")));
  ...
  // OD stop/thru energy correction branches
  TTree* trOutput = new TTree( "odstopthru", "OD Stop/Thru Tree" );
  ODStopThru* odst = new ODStopThru();
  odst->DefineBranches( trOutput );
  odst->SetVerbose( 0 );
  ...
  for (int n=0; n<str_filelist.size(); n++) {    
	...
	// Perform OD Stop/Thru Energy Correction
	// clear out last entry
	odst->ClearTrackList();
	// first import APFIT tracks
	for (int ring=0; ring<apcommul_.apnring; ring++) {
	  int pdg;
	  double vertex[3], dir[3];
	  double mom = apcommul_.apamom[ring];
	  std::cout << "ip: " << apcommul_.apip[ring] << std::endl;
	  pdg = apcommul_.apip[ring];
	  if (pdg==11) {
	    mom = appatsp2_.apmsamom[ring][1];
	  }
	  else {
	    mom = appatsp2_.apmsamom[ring][2];
	  }
	  
	  for (int v=0; v<3; v++) {
	    dir[v] = apcommul_.apdir[ring][v];
	    vertex[v] = apcommul_.appos[v];
	  }
	  odst->ImportTrack( pdg, vertex, dir, mom );
	}
	// second perform analysis
	odst->ProcessEvent();
	// dump results
	odst->PrintResultSummary();
	// save resutls
	odst->Fill();
      }// if read flag is ok.
    }//end of while loop
